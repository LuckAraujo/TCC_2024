    ...
    while True:
        ...
        if response.status_code == 200:
            ...
            for aluno in alunos:
                ...
                if aluno.get("CH_TOTAL") in ("0.00", "45.00","72.00","120.00", "318.00"):
                    continue
...
df = pd.DataFrame(alunos_filtrados)

adsa = df["ID_ALUNO"].nunique()
print(adsa)

# Agrupar por ID_ALUNO
df_alunos = df.groupby("ID_ALUNO").agg({
    "IDADE": "first",  
    "ANO_INGRESSO": "first",  
    "FORMA_INGRESSO": "first",  
    "ANO_EVASAO": "first",  
    "FORMA_EVASAO": "first",  
    "NATURALIDADE": lambda x: x.mode()[0] if not x.mode().empty else "Desconhecido",  
    "ETNIA": lambda x: x.mode()[0] if not x.mode().empty else "Desconhecido",
    "DEFICIENCIAS": "first",
    "COTAS": lambda x: x.mode()[0] if not x.mode().empty else "Desconhecido",
    # "CH_TOTAL": "sum",  
    # "MEDIA_FINAL": lambda x: x.mode()[0] if not x.isna().all() else "Nao informado",
    "BOLSA": lambda x: "Possuia" if "Possuia" in x.values else "Não Possuia"
}).reset_index()

df_alunos.to_csv("dados_alunos_unicos.csv", index=False, encoding="utf-8")

print(f"Dados de {len(df_alunos)} alunos salvos em 'dados_alunos_unicos.csv' com sucesso!")